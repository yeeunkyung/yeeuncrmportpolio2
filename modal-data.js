...
<div class="mnote">→ 카카오 채널 추가 고객 대상 CRM 메시지를 통해 회원가입 전환 유도</div>`},
...
// watch position (fixed)
(function(){
  const W = document.querySelector('.hero-watch-wrap');
  const img = document.getElementById('watch-a');
  if(W) W.style.cssText = 'position:absolute;left:20%;top:50%;transform:translateY(calc(-50% + 10%));width:55%;pointer-events:none;z-index:1;';
